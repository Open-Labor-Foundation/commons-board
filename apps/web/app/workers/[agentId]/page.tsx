...
// tags — truncate and cap so the header doesn't turn into a paragraph of run-on text.
function shortTasks(tasks: string[], max: number): string[] {
  return tasks.slice(0, max).map(t => (t.length > 70 ? t.slice(0, 67).trimEnd() + "…" : t));
}
...
export default function WorkerDetailPage() {
  ...
  const [chatMessages, setChatMessages] = useState<Array<{ role: "user" | "board"; content: string }>>([]);
  ...
  async function sendMessage() {
    ...
    // Build history from prior turns for multi-turn context
    const history = chatMessages.map(m => ({
      role: m.role === "user" ? "user" as const : "assistant" as const,
      content: m.content,
    }));

    const res = await apiFetch<{ reply: string; thinking: string | null }>(
      `/api/v1/workers/${agentId}/chat`,
      {
        method: "POST",
    ...
    if (res?.reply) {
      setChatMessages(p => [...p, { role: "board", content: res.reply }]);
    } else {
      ...
      <div style={{ padding: "16px 24px", borderBottom: "1px solid var(--border)", background: "var(--surface)", flexShrink: 0 }}>
        ...
        {worker.task_scope && worker.task_scope.length > 0 && (
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 8 }}>
            {shortTasks(worker.task_scope, 3).map(item => (
          ...
          <div style={{ padding: "10px 14px", borderTop: "1px solid var(--border)", background: "var(--surface)", flexShrink: 0 }}>
            {chatMessages.length > 0 && (
              <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 5 }}>
                <button onClick={() => setChatMessages([])} style={{ fontSize: 10, color: "var(--text-muted)", background: "none", border: "1px solid var(--border)", borderRadius: 4, padding: "1px 6px", cursor: "pointer" }}>
                  New conversation
                </button>
              </div>
            )}
