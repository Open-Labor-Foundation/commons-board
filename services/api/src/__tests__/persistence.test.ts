...
describe("persistence", () => {
  ...
  test("readJson rejects traversal in a key segment", () => {
    assert.throws(() => readJson("settings/../../../etc/passwd", null));
  });

  test("writeJsonAtomic rejects traversal in a key segment", () => {
    assert.throws(() => writeJsonAtomic("settings/../../../tmp/pwn", { evil: true }));
  });

  test("readJson rejects a bare '..' segment", () => {
