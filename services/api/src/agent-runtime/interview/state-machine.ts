...
export class InterviewStateMachine {
  ...
  finalize(): { assumptions: string; artifacts: ReturnType<typeof generateArtifacts> } {
    ...
    const corrections = this.answers.S9?.corrections;
    if (corrections) {
      for (const [section, value] of Object.entries(corrections) as Array<[InterviewSection, unknown]>) {
        const current = (this.answers as Record<string, unknown>)[section];
        if (current && typeof current === "object" && value && typeof value === "object") {
          (this.answers as Record<string, unknown>)[section] = {
    ...
    const artifacts = generateArtifacts(this.answers, this.orgId);
    this.finalized = true;
