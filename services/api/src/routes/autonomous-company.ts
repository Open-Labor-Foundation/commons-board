...
autonomousCompanyRouter.post("/signals", requireRole(["admin", "operator"]), (req: Request, res: Response) => {
  ...
  const signal = addMarketSignal(workspaceId, {
    ...
    replyRate: Number(body.replyRate ?? 0),
    meetingRate: Number(body.meetingRate ?? 0),
    conversionRate: Number(body.conversionRate ?? 0),
    revenueVelocity: Number(body.revenueVelocity ?? 0),
    costPerAcquisition: Number(body.costPerAcquisition ?? 0),
    bounceRate: Number(body.bounceRate ?? 0),
    unsubscribeRate: Number(body.unsubscribeRate ?? 0),
    complaintRate: Number(body.complaintRate ?? 0),
    landingConversionRate: Number(body.landingConversionRate ?? 0),
    pipelineGrowth: Number(body.pipelineGrowth ?? 0),
    revenueGrowth: Number(body.revenueGrowth ?? 0)
  });
...
autonomousCompanyRouter.post("/experiments", requireRole(["admin", "operator"]), (req: Request, res: Response) => {
  ...
  const experiment: ExperimentRecord = {
    id: randomUUID(),
    workspaceId,
    hypothesis: String(body.hypothesis ?? "improve conversion"),
    channel: String(body.channel ?? "outbound"),
...
autonomousCompanyRouter.get("/experiments", (req: Request, res: Response) => {
  const { workspaceId } = req.ctx!;
  res.status(200).json({ experiments: listExperiments(workspaceId) });
});
