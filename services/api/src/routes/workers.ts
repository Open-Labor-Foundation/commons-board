 ...
 *   POST /:agentId/task        — dispatch a task to a specific worker
 *   POST /:agentId/chat        — send a message routed to the worker's chair domain
 */
  ...
  for (const chair of chairs) {
    for (const worker of chair.worker_agents) {
      ...
      workers.push({
        ...
        status: workerStatus(worker.agent_id, actions, approvals, chair),
...
workersRouter.get("/:agentId", (req: Request, res: Response) => {
  ...
  res.status(200).json({
    ...
    status: workerStatus(agentId, actions, approvals, chair),
    current_task: currentTask,
...
// ---------------------------------------------------------------------------
// POST /api/v1/workers/:agentId/task — dispatch a task to a worker
// ---------------------------------------------------------------------------
