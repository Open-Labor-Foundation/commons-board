...
> {
  ...
  try {
    const raw = await completeText(input.workspaceId, system, userPrompt, { max_tokens: 2000 });
    ...
    return { ok: false, error: "synthesis_error", detail: err instanceof Error ? err.message : "unknown error" };
  }
}
