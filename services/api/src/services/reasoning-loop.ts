...
function criticAgent(input: { intentConfidence: number; domainPass: boolean; deliverablePass?: boolean }): CriticStep {
  const issues: string[] = [];
  if (input.intentConfidence < 0.45) issues.push("intent_confidence_below_floor");
...
export function runReasoningLoop(input: {
  ...
  const critic = criticAgent({ intentConfidence: input.intentConfidence, domainPass: input.domainPass, deliverablePass: input.deliverablePass });
